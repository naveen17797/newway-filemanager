...
$loader->assign('WRITE_ACCESS_BOOL', ($newway_user_create_access ? '' : 'disabled'));
$loader->assign('DELETE_ACCESS_BOOL', ($newway_user_delete_access ? '' : 'disabled'));
...
if (isset($_GET['directory'])) {
    if (!empty($_GET['directory'])) {
        $upload_directory = $_GET['directory'] . ((substr($_GET['directory'], -1) === '/') ? '/' : '');
    }
} else {
...
if ($newway_user_read_access) {
    if (!empty($_GET['directory'])) {
        $newway_root_directory = $_GET['directory']. ((substr($_GET['directory'], -1) === '/') ? '/' : '');
        $path = pathinfo($newway_root_directory);
        if ($path['dirname'] !== '.') {
            echo "<div class='btn-group'><a href=index.php?directory=".$path['dirname']. (
                ($path['dirname'] === '..') ? '/' : ''
            ) . " class='btn btn-info btn-responsive'><i class='fa fa-arrow-left'></i> Back</a></div>";
        }
    }
